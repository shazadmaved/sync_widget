...
class ArrowPath {
  ...
  static Path _make(Path path, double tipLength, double tipAngle,
    ...
    tan = lastPathMetric.getTangentForOffset(lastPathMetric.length * 0.5);

    final Offset originalPosition = tan!.position;

    if (isAdjusted && lastPathMetric.length * 0.5 > 10) {
      Tangent tanBefore =
          lastPathMetric.getTangentForOffset(lastPathMetric.length * 0.5 - 5)!;
